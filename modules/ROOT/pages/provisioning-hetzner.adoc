...
----
+
. Launch a server. You can create a server from the snapshot created in the previous in the web console and skip passing user data if you just want SSH access. This provides an easy way to test out FCOS without first creating an Ignition config. To use an Ignition configuration, pass it to the VM as user data as below.
+
