...
- Take the latest ISO image from the https://getfedora.org/coreos/download?tab=metal_virtualized&stream=stable[download page] or using podman (download stable by default, see https://coreos.github.io/coreos-installer/cmd/download/[documentation] for options):
[source, bash]
----
