...
steps as described above to get the live environnement, simply adding the correct kernel arguments.


- Mount the iscsi target from the live environnement :
[source, bash]
----
...
NOTE: If you are using iBFT you can load the initiator and target values from the firmware : `--append-karg rd.iscsi.firmware=1 --append-karg ip=ibft`
