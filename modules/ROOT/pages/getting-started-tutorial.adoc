...
- You have libvirtd installed and active on out system.
...
Along with the extracted `.qcow2` image, you would also need two other programs in order to transpile and validate the ignition scripts, which are:
...
Additionally we offer the ignition-validate sub-utility that can be used to verify Ignition config and catch issues before launching a machine.
...
- Add a systemd dropin to override the norman `serial-getty@ttyS0.service`. The override will make the service automatically log the `core` user in to the serial console of the booted machine.
...
You can see the full configuration for serial-getty@ttyS0.service by using systemctl cat serial-getty@ttyS0.service.
...
[core@localhost]$ sudo docker info
...
This script uses icanhazip.com to update the issuegen from console-login-helper-messages to output the node's public IPv4 address on the serial console during bootup.
