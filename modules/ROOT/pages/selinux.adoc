...
----
+
After this command, the output from `ostree admin config-diff | grep policy` should no longer indicate the policy is modified.

If your policy still appears to be modified, you can try the following approach:

. Remove the SELinux policy and copy in the default policy:
+
[source,bash]
----
