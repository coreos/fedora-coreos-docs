...
== Basic installation from live ISO
...
Proposed addition to https://docs.fedoraproject.org/en-US/fedora-coreos/bare-metal/

== Automated ISO installs with Ignition embedding
...
As noted above for Live PXE, the live environment (including ISO) does not have to actually perform 
a persistent installation.  You can boot it from a read-only medium (physical ISO, USB stick with a physical read-only switch),
and do everything you do on any other Fedora CoreOS environment.
Each boot will re-run the Ignition config.

The mechanism to do this is `coreos-installer ignition iso embed`, which will create a new `.iso` file that combines your configuration with the ISO.
...
Some documentation on this is on the upstream installer site: https://coreos.github.io/coreos-installer/customizing-install/
...
First, generate your target configuration `target.ign`.  Then, using e.g. `fcct`, embed it as a file in your ISO ignition, and use a custom systemd unit to pass it to `coreos-installer`:
...
Commonly bare metal systems will have a diversity of hardware - some systems may have NVMe drives `/dev/nvme*`, whereas others have `/dev/sd*` for example.
You will almost certainly have to template the value of `/dev/sda` above.
