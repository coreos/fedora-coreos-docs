...
WARNING: Support for Confidential Computing is working in progress in Fedora CoreOS
...
To launch a confidential FCOS instance should specify the confidential compute type, and a related https://learn.microsoft.com/en-us/azure/confidential-computing/virtual-machine-options[machine type] that supports confidential compute.
...
# Specify the size that supports confidence (using AMD SEV-SNP)
...
# Create VM with confidence type using generated Gallery image and custom-data
az vm create \
    --name "${vm_name}" \
    --resource-group $az_resource_group \
    --size "${vm_size}" \
...
Note: We pass parameter `--enable-secure-boot true` to enable the secure boot, update the value to `false` to disable secure boot.

Note: If you want to get the full console log to debug or check, should append parameter `--boot-diagnostics-storage ${az_storage_account}`
